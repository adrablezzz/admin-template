<template>
  <Modal v-model="isModal" width="600" title="视频审核" :mask-closable="false">
    <Form
      ref="formFieldRef"
      :model="formItem"
      :rules="ruleValidate"
...
const ruleValidate = ref({
  comment: [{ required: true, message: "审核意见不能为空", trigger: "change" }],
});
...
const handlePass = (status: number): void => {
  ...
  formFieldRef?.value?.validate((v: boolean) => {
    if (v) {
      ...
      let params = {
        ...formItem.value,
        ids: formItem.value.ids.join(","),
      };
      videoModel.checkVideo(params).then((_da: any) => {
