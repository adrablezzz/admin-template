<template>
  <div class="tag-nav">
    ...
    <div class="right">
      <div class="arrow" @click="handleScroll(1)">
...
const routes = computed<any>(() => store.getRoutesLog);
...
// 组件
const tagWrapper = ref<null | HTMLDivElement>(null);
const tagListRef = ref<null | HTMLDivElement>(null);
...
const handleScroll = (v: number): void => {
  ...
  if (v === 0) {
    ...
    if (
      -tagListLeft.value + (tagWrapper?.value?.offsetWidth || 0) >=
      (tagListRef?.value?.scrollWidth || 0)
    ) {
      tagListLeft.value += step;
    }
    tagListLeft.value -= step;
...
const handleTagClose = (_event: any, name: string) => {
