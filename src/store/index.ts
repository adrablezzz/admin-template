  ...
  "UseStoreId",
  {
    ...
    actions: {
      addRouteLog(route: Route) {
        if (this.routesLog.findIndex((r) => r.name == route.name) === -1) {
          this.routesLog.push(route);
          sessionStorage.setItem(
            ROUTESLOG,
            circularJSON.stringify(this.routesLog)
          );
        }
      },
      delRouteLog(name: string): number {
        let index: number = this.routesLog.findIndex((r) => r.name == name);
        if (index !== -1) {
      ...
      clearRouteLog(name?: string) {
        this.routesLog = this.routesLog.filter(
          (r) => r.name == initRoute || r.name == name
        );
        sessionStorage.setItem(
          ROUTESLOG,
          circularJSON.stringify(this.routesLog)
      ...
      login(token: string, userInfo: UserInfo) {
        ...
        Cookies.set("userInfo", circularJSON.stringify(userInfo), {
          expires: 7,
        });
        this.token = token;
